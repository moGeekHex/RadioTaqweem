...
function timeDiff(t){
  return moment().diff(t, 'seconds')
}
function isBefore(t){
  return moment().isBefore(t)
}

function formatTime(seconds){
  return   '-'+ Math.abs(Math.floor(moment.duration(seconds,'seconds').hours())) + ':' + Math.abs(moment.duration(seconds,'seconds').minutes()) /*+ ':' + Math.abs(moment.duration(seconds,'seconds').seconds())*/ ;
}
