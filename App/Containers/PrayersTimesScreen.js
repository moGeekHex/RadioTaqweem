...
todayKey = (moment=moment())=>{
  return `currentHijriDay:${moment.year()}${moment.format("MM")}${moment.date()}`
}
  ...
  async componentDidMount() {
   ...
   try{
    ...
    await Notification.createNotification()
     const currentHijriDayCache = await AsyncStorage.getItem(this.todayKey(moment()))
        if(currentHijriDayCache){
    this.setState({ currentHijriDay:JSON.parse( currentHijriDayCache)}) 
  return     
  }
...
const [todaySnapshot,offsetSnapShot] =  await  api.getTodayAndOffsetPromise(moment().year(), moment().format("MM"), moment().date())
      ...
      currentHijriDay&&  currentHijriDay.hijri &&  this.setState({ currentHijriDay: appliedDate}, async () => {
      
        await AsyncStorage.setItem(this.todayKey(this.state.selectedDay), JSON.stringify(appliedDate));
      })
    ...
    return (
      ...
      >
        ...
        <ImageBackground
          ...
          resizeMode="contain"
        >
          {salah && <View key={`${this.state.keyCount}_key`} style={{ flexDirection: 'row-reverse', justifyContent: 'space-evenly',alignItems:'center', width: Metrics.screenWidth - 20, borderWidth: 1, borderColor: 'lightgrey', borderRadius: 5, alignSelf: 'center', height: responsiveHeight(10) + 10, marginTop: 20 }}>
