...
export default class PrayersTimesScreen extends Component {
  ...
  console.log('currentMiladiDay',currentMiladiDay)
  const m = ('0'+currentMiladiDay.selectedMonth).slice(-2)
    const salah = salawat[m].find(salah => salah.day == currentMiladiDay.selectedDay);
...
const hideHighlightCurrentSalah= this.props.hideHighlightCurrentSalah
const withoutTimer = this.props.withoutTimer || false
...
const {currentMiladiDay} = this.props
    ...
    <View>
          ...
          {salah && <View style={{ flexDirection: 'row-reverse', justifyContent: 'space-evenly',/* width: Metrics.screenWidth - 20, borderWidth: 1, borderColor: 'lightgrey', borderRadius: 5, alignSelf: 'center', height: responsiveHeight(10) + 10, marginTop: 20 */}}>
            ...
            {fajer && <PrayerCard withoutTimer={withoutTimer} prayerName="الفجر" time={fajer} timeMoment={this.salahMoment.fajer} nextSalahName={salahName} isNextPrayer={this.isNextPrayer(salahName, [salwatNames[0]])} fontSizeFactor={fontSizeFactor}  hideHighlightCurrentSalah={hideHighlightCurrentSalah} />}
            {sunrise && <PrayerCard  withoutTimer={withoutTimer} prayerName="الشروق" time={sunrise} timeMoment={this.salahMoment.sunrise} nextSalahName={salahName} fontSizeFactor={fontSizeFactor}  hideHighlightCurrentSalah={hideHighlightCurrentSalah} />}
            {dhuhur && <PrayerCard withoutTimer={withoutTimer} prayerName="الظهر" time={dhuhur} timeMoment={this.salahMoment.dhuhur} nextSalahName={salahName} isNextPrayer={this.isNextPrayer(salahName, [salwatNames[1], salwatNames[2]])} fontSizeFactor={fontSizeFactor} hideHighlightCurrentSalah={hideHighlightCurrentSalah} />}
            {asr && <PrayerCard withoutTimer={withoutTimer} prayerName="العصر" time={asr} timeMoment={this.salahMoment.asr}  nextSalahName={salahName} isNextPrayer={this.isNextPrayer(salahName, [salwatNames[3]])} fontSizeFactor={fontSizeFactor} hideHighlightCurrentSalah={hideHighlightCurrentSalah} />}
            {maghreb && <PrayerCard withoutTimer={withoutTimer} prayerName="المغرب" time={maghreb} timeMoment={this.salahMoment.maghreb}  nextSalahName={salahName} isNextPrayer={this.isNextPrayer(salahName, [salwatNames[4]])} fontSizeFactor={fontSizeFactor} hideHighlightCurrentSalah={hideHighlightCurrentSalah} />}
            {Ishaa && <PrayerCard withoutTimer={withoutTimer} prayerName="العشاء" time={Ishaa} timeMoment={this.salahMoment.Ishaa} nextSalahName={salahName} isNextPrayer={this.isNextPrayer(salahName, [salwatNames[5]])} fontSizeFactor={fontSizeFactor} hideHighlightCurrentSalah={hideHighlightCurrentSalah} />}
