...
class VirtueScreen extends Component {
  ...
  virtueKey = (moment = moment())=>{
    return `${moment.year()}${moment.format("MM")}${moment.date()}`
  }
  async componentDidMount() {
    
  
     this.setState({selectedDate:moment()})
     const dayKey = this.virtueKey(moment())
    const today = await AsyncStorage.getItem(dayKey);
    if (today) {
      let todayjson = JSON.parse(today)
      this.setState({ today: todayjson,virtue:todayjson.virtue ,loading: false });
   return 
     }
  try{
   this.getVirtue(moment())
  }catch(e){
    console.warn(e)
  }
  ...
  mcb = ss => {
    const {key,selectedDate} = this.state
    this.setState({ key:key+1,virtue:ss.val().virtue,today: ss.val(),loading: false  })
      if(moment().diff(selectedDate, 'days') == 0){
    AsyncStorage.setItem(this.virtueKey(selectedDate), JSON.stringify(ss.val()));
      }
  }
  increaseDay =()=>{
      ...
      >
        ...
        <View style={{ flex: 4 }}>
          {loading &&
            ...
            { !loading && <View style={[styles.container,{flexDirection:'row'}]}>
            { false &&<Entypo onPress={this.increaseDay} style={{ fontSize: this.props.fontSizeFactor * responsiveFontSize(4.5), alignSelf: 'center', }} name="chevron-thin-left" />}


            <ScrollView style={[styles.container, { marginVertical: 10 }]}>
            ...
            {false &&<Entypo onPress={this.decreaseDay} style={{ fontSize: this.props.fontSizeFactor * responsiveFontSize(4.5), alignSelf: 'center' }} name="chevron-thin-right" />}
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigation(VirtueScreen))
