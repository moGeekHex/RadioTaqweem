...
import { AsyncStorage, ScrollView, TouchableOpacity,
   ImageBackground, Linking ,Share,} from 'react-native'
...
class SettingsScreen extends Component {
  ...
  async componentDidMount() {
    await this.loadNotificationPref()
    await  this.loadMoazenPref()
  ...
  changeIsNotificationsOn = async (value) => {
    ...
    await AsyncStorage.setItem('isNotificationsOn', value?'true':'false');
        ...
        case 'live':
          ...
          case 'website':
              ...
              case 'app':
                    Share.share({
                    message:
                    'https://play.google.com/store/apps/details?id=com.perfectfit.qurankareem',
                  }).then(()=>{});
  ...
  changeNotificationPref = async (value)=>{
    try{
      ...
      const result = await AsyncStorage.setItem('notificationPref',value?'true':'false')
  ...
  loadNotificationPref = async ()=>{
      ...
      this.setState({notificationPref:result =='true'?true:false})
